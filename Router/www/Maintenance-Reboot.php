...
<p>&nbsp;</p>
  ...
  <div id="ContentTitle">
  <span>Reboot PhotoFrame</span></div>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" enctype="application/x-www-form-urlencoded" id="reboot">
      <fieldset>
        <table width="100%" border="0">
          <tr>
            <td>&nbsp;</td>
		<p>This will reboot the Photo Frame. You will be unable to connect, while it is rebooting.</p>
          </tr>
          <tr>
...
<!-- ********************************************************************************************************************** -->
  <?php 
	if($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['reboot'])) {
		...
		session_start();
		session_destroy();
		logmessage("Rebooting Photo Frame Router.");
