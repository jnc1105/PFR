########################################################################################
# Bootstrap - Preparing the Disguised Technologies Kernel
########################################################################################
apt-get update
apt-get -y install apt-utils
apt-get -y install git-core
useradd dtech
echo 'dtech:Dt3ch20!'|chpasswd
usermod -a -G sudo dtech
mkdir /dtech
mkdir /dtech/www
git clone https://github.com/ronnyvdbr/Raspberry-Wifi-Router.git /dtech
cp /root/.profile /dtech/.profile
cp /root/.bashrc /dtech/.bashrc
cp /dtech/other/images/splash.png /etc
sed -i 's/pi:x:1000:1000::\/dtech:\/bin\/sh/pi:x:1000:1000::\/dtech:\/bin\/bash/g' /etc/passwd   # Probably Wrong
sed -i 's/# export LS_OPTIONS=/export LS_OPTIONS=/g' /dtech/.bashrc
sed -i 's/# eval/eval/g' /dtech/.bashrc
sed -i "s/# alias ls=/alias ls=/g" /dtech/.bashrc
sed -i "s/# alias ll=/alias ll=/g" /dtech/.bashrc
sed -i "s/# alias l=/alias l=/g" /dtech/.bashrc
chown -R pi /dtech
chgrp -R pi /dtech
chmod -R 755 /dtech

########################################################################################
# Update - Making sure that your Device is the latest version.
########################################################################################
...
echo 'root:Dt3ch20!'|chpasswd
echo "Europe/Dublin" > /etc/timezone     #Fix
...
sudo rm -R /var/www
sudo ln -s /dtech/www /var/www
sudo chown dtech:www-data /var/www
sudo chown -R dtech:www-data /dtech/www
sudo chmod g+w /dtech/www/routersettings.ini
sudo chmod 775 /var/www
sudo usermod -a -G www-data dtech
...
sudo cp /dtech/defconfig/wr_commands /etc/sudoers.d/wr_commands
sudo chmod 644 /etc/sudoers.d/wr_commands
sudo cp /dtech/defconfig/interfaces /etc/network/interfaces
sudo chgrp www-data /etc/network/interfaces
sudo chmod g+w /etc/network/interfaces
sudo cp /dtech/defconfig/ntp.conf /etc/ntp.conf
sudo chgrp www-data /etc/ntp.conf
sudo chmod g+w /etc/ntp.conf
sudo cp /dtech/defconfig/dnsmasq.conf /etc/dnsmasq.conf
sudo chgrp www-data /etc/dnsmasq.conf
sudo chmod g+w /etc/dnsmasq.conf
sudo cp /dtech/defconfig/hostapd.conf /etc/hostapd/hostapd.conf
...
sudo cp /dtech/defconfig/routersettings.ini /var/www/routersettings.ini
...
sudo cp /dtech/defconfig/cmdline.txt /boot/cmdline.txt
...
echo 'mysql-server mysql-server/root_password password Dt3ch20!' | debconf-set-selections
echo 'mysql-server mysql-server/root_password_again password Dt3ch20!' | debconf-set-selections
...
mysql --host=localhost --user=root --password=Dt3ch20! radius < /etc/freeradius/sql/mysql/schema.sql
mysql --host=localhost --user=root --password=Dt3ch20! radius < /etc/freeradius/sql/mysql/admin.sql
echo "insert into radcheck (username, attribute, op, value) values ('user', 'Cleartext-Password', ':=', 'password');" | mysql --host=localhost --user=root --password=Dt3ch20! radius
...
########################################################################################
echo 'create database login;' | mysql --host=localhost --user=root --password=Dt3ch20!
echo " \
CREATE TABLE users ( \
  ...
  PRIMARY KEY  (id) \
) ;" | mysql --host=localhost --user=root --password=Dt3ch20! --database login
...
echo "INSERT INTO users (username,password) VALUES('admin','Dt3ch20!');" | \
mysql --host=localhost --user=root --password=Dt3ch20! --database login
